...
def generate_full_trajectory() -> list[dict[str, float]]:
    x = X0
    y = Y0
    z = Z0
...
def run(output_csv: Path) -> None:
    ...
    rows = generate_full_trajectory()
    rows = attach_window_scores(rows)
    ...
    first_alert_after_switch = None
    for row in rows:
        if row["step"] >= SWITCH_STEP and row["alert_flag"] == 1:
            first_alert_after_switch = row["step"]
            break
    ...
    with output_csv.open("w", encoding="utf-8", newline="") as f:
        ...
        print("OMNIABASE LORENZ REALTIME ALERT V1")
        print("-" * 88)
        ...
        print("-" * 88)
        if first_alert_after_switch is None:
            print("first_alert_after_switch=None")
        else:
            delay = first_alert_after_switch - SWITCH_STEP
            print(f"first_alert_after_switch={first_alert_after_switch}")
            print(f"alert_delay_steps={delay}")
            print(f"alert_delay_time={delay * float(DT):.6f}")
        print("-" * 88)
        print(f"Done. Wrote {output_csv}")


if __name__ == "__main__":
    run(Path("outputs/lorenz_realtime_alert_v1.csv"))