...
# Response initial state (different on purpose)
X0_R = Decimal("5.0")
Y0_R = Decimal("5.0")
Z0_R = Decimal("5.0")
...
def drive_response_derivatives(
    ...
    # response system receives coupling only on x equation
...
def rk4_coupled_step(
    ...
    s2 = [
        v + dt * kv / Decimal("2")
        for v, kv in zip([xd, yd, zd, xr, yr, zr], k1)
    ]
    k2 = drive_response_derivatives(*s2, coupling)

    s3 = [
        v + dt * kv / Decimal("2")
        for v, kv in zip([xd, yd, zd, xr, yr, zr], k2)
    ]
    k3 = drive_response_derivatives(*s3, coupling)

    s4 = [
        v + dt * kv
        for v, kv in zip([xd, yd, zd, xr, yr, zr], k3)
    ]
...
def generate_trajectory(coupling: Decimal) -> list[tuple[Decimal, Decimal, Decimal, Decimal, Decimal, Decimal]]:
...
def summarize_coupling(coupling: Decimal) -> dict[str, float]:
    traj = generate_trajectory(coupling)
    ...
    # Normalize each error signal to [0,1] for multibase signature extraction
    ...
    # tail behavior matters most for synchronization
...
def run(output_csv: Path) -> None:
    ...
    rows = [summarize_coupling(c) for c in COUPLING_VALUES]
    ...
    with output_csv.open("w", encoding="utf-8", newline="") as f:
        ...
        print("OMNIABASE LORENZ SYNCHRONIZATION V1")
        print("-" * 96)
...
if __name__ == "__main__":
    run(Path("outputs/lorenz_synchronization_v1.csv"))